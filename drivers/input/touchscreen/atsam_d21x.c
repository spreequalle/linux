...
/* 100 points for touch raw data and 1 for ALS raw data. */
#define ATSAM_D21X_RAW_DATA_NUM		101
...
#define ATSAM_D21X_ALS_VALUE_ADDR	0x08
...
#define ALS_POLLING_WORK_DELAY_MS	2000  /* ms */
#define ALS_REPORT_THRESHOLD		0
#define ALS_WORK_COUNT_FOR_ONE_MIN	30
/* 60 * 24, cache ALS value every 1 minute. */
#define CACHED_ALS_VALUE_COUNT		1440
#define INVALID_ALS_VALUE		4096

#define NUM_OF_RETRIES			5

struct atsam_d21x_als {
	int als_work_count;
	int next_als_index;
	u16 als_value;
	u16 last_reported_als;
	u16 cached_als_value[CACHED_ALS_VALUE_COUNT];
	/* workqueue for ALS polling */
	struct delayed_work work_als;
	struct workqueue_struct *workqueue_als;
	struct mutex mutex_for_als;
};
...
struct atsam_d21x {
	 ...
	 * There are 100 touch points and 1 ALS entry.
	 * Each one is 2 bytes.
	 */
	u8 raw_data[ATSAM_D21X_RAW_DATA_SIZE];

	/* ALS structure */
	struct atsam_d21x_als als;
	/* debugfs entry */
	struct dentry *cached_als_file;
};
...
static ssize_t atsam_d21x_als_value_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct atsam_d21x *data = i2c_get_clientdata(client);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)data->als.als_value);
}
...
static DEVICE_ATTR(als_value, S_IRUGO, atsam_d21x_als_value_show, NULL);
...
static void atsam_d21x_start(struct atsam_d21x *data)
{

}

static void atsam_d21x_stop(struct atsam_d21x *data)
{

}

static int atsam_d21x_input_open(struct input_dev *dev)
{
	struct atsam_d21x *data = input_get_drvdata(dev);

	atsam_d21x_start(data);

	return 0;
}

static void atsam_d21x_input_close(struct input_dev *dev)
{
	struct atsam_d21x *data = input_get_drvdata(dev);

	atsam_d21x_stop(data);
}
...
static void atsam_d21x_cache_als_value(struct atsam_d21x_als *als)
{
	mutex_lock(&als->mutex_for_als);
	/* Mod first to make sure next_als_index is in range. */
	als->next_als_index = als->next_als_index % CACHED_ALS_VALUE_COUNT;
	als->cached_als_value[als->next_als_index] = als->als_value;
	als->next_als_index = (als->next_als_index + 1) %
				CACHED_ALS_VALUE_COUNT;
	/* The array is unlikely to be filled up because of device daily reboot.
	 * In case it's full and need to write from the beginning, we need a way
	 * to tell where is the delimiter to the previous values, so always
	 * write an invalid value to next index so when there is on invalid
	 * value between two normal value, that's where the overwrite happen.
	 */
	als->cached_als_value[als->next_als_index] = INVALID_ALS_VALUE;
	mutex_unlock(&als->mutex_for_als);
}

static void atsam_d21x_als_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct atsam_d21x_als *als_p = container_of(dwork, struct atsam_d21x_als,
						work_als);
	struct atsam_d21x *data = container_of(als_p, struct atsam_d21x, als);
	struct i2c_client *client = data->client;
	u8 val[2];
	u8 status = 0;
	int ret;
	...
	ret = atsam_d21x_i2c_read_data(client, ATSAM_D21X_ALS_VALUE_ADDR,
						sizeof(val), val);
	if (ret) {
		dev_err(&client->dev, "%s: failed to read als value.\n",
			__func__);
		goto exit_rearm_work;
	}

	als_p->als_value = le16_to_cpu(*(u16*)val);  /* little endian */
	if (abs(als_p->als_value - als_p->last_reported_als) >
		ALS_REPORT_THRESHOLD) {
		als_p->last_reported_als = als_p->als_value;
		input_event(data->input_dev, EV_MSC, MSC_SCAN,
				als_p->als_value);
		input_sync(data->input_dev);
	}

	als_p->als_work_count++;
	/* Cache als_value approximately every 1 minute. */
	if (als_p->als_work_count >= ALS_WORK_COUNT_FOR_ONE_MIN) {
		atsam_d21x_cache_als_value(als_p);
		als_p->als_work_count = 0;
	}

exit_rearm_work:
	/* Rearm workqueue itself. */
	queue_delayed_work(als_p->workqueue_als, &als_p->work_als,
			msecs_to_jiffies(ALS_POLLING_WORK_DELAY_MS));
}

#ifdef CONFIG_DEBUG_FS
static int cached_als_debug_show(struct seq_file *s, void *data)
{
	struct atsam_d21x_als *als = s->private;
	int index;

	mutex_lock(&als->mutex_for_als);
	index = (als->next_als_index - 1 + CACHED_ALS_VALUE_COUNT) %
			CACHED_ALS_VALUE_COUNT;

	seq_printf(s, "Cached ALS values:\n");

	for (; index >= 0; --index)
		seq_printf(s, "%d ", als->cached_als_value[index]);

	for (index = CACHED_ALS_VALUE_COUNT - 1; index >= 0; --index) {
		if (als->cached_als_value[index] == INVALID_ALS_VALUE)
			break;
		seq_printf(s, "%d ", als->cached_als_value[index]);
	}
	seq_printf(s, "\n");
...
{
	return single_open(file, cached_als_debug_show, inode->i_private);
}

static const struct file_operations cached_als_debug_fops = {
	.open		= debug_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
...
static int atsam_d21x_create_debugfs(struct atsam_d21x *data)
{
	struct i2c_client *client = data->client;
	data->cached_als_file = debugfs_create_file("cached_als", 0444, NULL,
					  &data->als, &cached_als_debug_fops);
	if (!data->cached_als_file)
		return -ENODEV;

	return 0;
}

static void atsam_d21x_remove_debugfs(struct atsam_d21x *data)
{
	if (data->cached_als_file)
		debugfs_remove(data->cached_als_file);
}
#else
static int atsam_d21x_create_debugfs(struct atsam_d21x *data)
{
...
static inline void atsam_d21x_remove_debugfs(struct atsam_d21x *data)
{
}
				...
				const struct i2c_device_id *id)
{
	...
	input_dev->open = atsam_d21x_input_open;
	input_dev->close = atsam_d21x_input_close;
	...
	/* MSC_SCAN is used for ALS event. */
	...
	/* ALS initialization */
	data->als.als_work_count = 0;
	data->als.next_als_index = 0;
	data->als.als_value = 0;
	data->als.last_reported_als = 0;

	/* Initialize cached_als_value array. */
	for (index = 0; index < CACHED_ALS_VALUE_COUNT; ++index)
		data->als.cached_als_value[index] = INVALID_ALS_VALUE;

	mutex_init(&data->als.mutex_for_als);
	data->als.workqueue_als =
			create_singlethread_workqueue("workqueue_als");
	if (data->als.workqueue_als == NULL)
		return -ENOMEM;
	INIT_DELAYED_WORK(&data->als.work_als, atsam_d21x_als_work);
	queue_delayed_work(data->als.workqueue_als,
			&data->als.work_als,
			msecs_to_jiffies(ALS_POLLING_WORK_DELAY_MS));

	error = atsam_d21x_create_debugfs(data);
	if (error) {
		dev_err(&client->dev, "%s: failed to create debugfs.",
			__func__);
		destroy_workqueue(data->als.workqueue_als);
		goto err_unregister_device;
	}
...
static int atsam_d21x_remove(struct i2c_client *client)
{
	...
	atsam_d21x_remove_debugfs(data);
	cancel_delayed_work_sync(&data->als.work_als);
	flush_workqueue(data->als.workqueue_als);
	destroy_workqueue(data->als.workqueue_als);
