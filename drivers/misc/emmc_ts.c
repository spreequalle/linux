...
static char fts_dev_node[] = "/dev/block/mmcblk0pxx";
...
static int erase_block(struct emmc_info *emmc, loff_t off)
{
	...
	for (i = 0; i < cnt; i++)
		if (filp->f_op->write(filp, buf, sizeof(buf), &filp->f_pos) < sizeof(buf)) {
		       ...
		       const void *buf, size_t size)
{
	...
	if (filp->f_op->write(filp, buf, size, &filp->f_pos) < size) {
		printk(KERN_ERR DRV_NAME ": failed to write %s\n", fts_dev_node);
		ret = -3;
		goto err2;
	}
...
static int flash_read(struct emmc_info *emmc, loff_t off, void *buf, size_t size)
{
	...
	if (filp->f_op->read(filp, buf, size, &filp->f_pos) < size) {
...
static int get_fts_dev_node(void)
{
	...
	sprintf(fts_dev_node, "/dev/block/mmcblk0p%d", dev_id);
