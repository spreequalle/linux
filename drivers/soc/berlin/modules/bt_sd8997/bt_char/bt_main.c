...
bt_init_module(void)
{
	...
	if ((boot_mode) &&  (0 == strcmp(boot_mode,"recovery"))) {
		PRINTM(MSG, "BT: recovery mode; exiting");
		return -EACCES;
	}
...
module_param(boot_mode, charp, 0);
MODULE_PARM_DESC(boot_mode, "Specify 'normal' or 'recovery'");
