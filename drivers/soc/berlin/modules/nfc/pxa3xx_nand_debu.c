...
MODULE_PARM_DESC(plane_cmdline, "command line for type of different partitions");
...
{ "DEFAULT FLASH",      0,      0,  4,   0, 2048,  8,  8, 0,    0, &timing[0], 0 },
{ "64MiB 16-bit",  0x46ec, 0xffff,  2,  32,  512, 16, 16, 1, 4096, &timing[1], 0 },
{ "256MiB 8-bit",  0xdaec, 0xffff,  4,  64, 2048,  8,  8, 1, 2048, &timing[1], 0 },
{ "4GiB 8-bit",    0xd7ec, 0xb655,  4, 128, 4096,  8,  8, 4, 8192, &timing[1], 0 },
{ "4GiB 8-bit",    0xd7ec, 0x29d5,  4, 128, 4096,  8,  8, 8, 8192, &timing[1], 0 },
{ "128MiB 8-bit",  0xa12c, 0xffff,  4,  64, 2048,  8,  8, 1, 1024, &timing[2], 0 },
{ "128MiB 16-bit", 0xb12c, 0xffff,  4,  64, 2048, 16, 16, 1, 1024, &timing[2], 0 },
{ "Toshiba 128MiB 8-bit",  0xf198, 0x1580,  4, 64, 2048,  8,  8, 32, 1024, &timing[6], 0},
{ "Power Chip 256MiB 8-bit",  0xdac8, 0x9590, 4, 64, 2048,  8,  8, 16, 2048, &timing[6], 0 },
{ "Power Chip 512MiB 8-bit",  0xdcc8, 0x9590, 4, 64, 2048,  8,  8, 16, 4096, &timing[6], 0 },
{ "Toshiba 256MiB 8-bit",  0xda98, 0x1590, 4, 64, 2048,  8,  8, 48, 2048, &timing[6], 0 },
{ "Toshiba 512MiB 8-bit",  0xdc98, 0x1591, 4, 64, 2048,  8,  8, 32, 4096, &timing[6], 0 },
{ "Hynix 256MiB 8-bit",  0xdaad, 0x9590, 4, 64, 2048,  8,  8, 32, 2048, &timing[6], 0 },
{ "512MiB 8-bit",  0xdc2c, 0xffff,  4,  64, 2048,  8,  8, 1, 4096, &timing[2], 0 },
{ "512MiB 16-bit", 0xcc2c, 0xffff,  4,  64, 2048, 16, 16, 1, 4096, &timing[2], 0 },
{ "256MiB 16-bit", 0xba20, 0xffff,  4,  64, 2048, 16, 16, 1, 2048, &timing[3], 0 },
{ "2GiB 8-bit",    0xd5ec, 0xb614,  4, 128, 4096,  8,  8, BCH_STRENGTH, 4096, &timing[4], 1 },
{ "2GiB 8-bit",    0xd5ec, 0x7284,  4, 128, 8192,  8,  8, 48, 2048, &timing[4], 0 },
{ "2GiB 8-bit",    0xd598, 0x3284,  4, 128, 8192,  8,  8, 80, 2048, &timing[5], 1 },
{ "2GiB 8-bit",    0x482c, 0x4a04,  4, 256, 4096,  8,  8, 48, 2048, &timing[6], 1 },
{ "2GiB 8-bit",    0xd5ad, 0xda94,  4, 256, 8192,  8,  8, 80, 1024, &timing[6], 1 },
{ "4GiB 8-bit",    0xd7ec, 0x7A94,  4, 128, 8192,  8,  8, 48, 4096, &timing[5], 1 },
{ "4GiB 8-bit",    0xd7ec, 0x7e94,  4, 128, 8192,  8,  8, 80, 4096, &timing[5], 1 },
{ "8GiB 8-bit",    0xdeec, 0x7ad5,  4, 128, 8192,  8,  8, 48, 4096, &timing[5], 1 },
{ "4GiB 8-bit",    0xd7ad, 0x9A94,  4, 256, 8192,  8,  8, 48, 2048, &timing[5], 1 },
{ "4GiB 8-bit",    0x682c, 0x4a04,  4, 256, 4096,  8,  8, 48, 4096, &timing[6], 1 },
{ "4GiB 8-bit",    0x682c, 0x4604,  4, 256, 4096,  8,  8, 48, 4096, &timing[6], 1 },
{ "8GiB 8-bit",    0x882c, 0x4b04,  4, 256, 8192,  8,  8, 48, 4096, &timing[6], 1 },
{ "8GiB 8-bit",    0xdead, 0xda94,  6, 256, 8192,  8,  8, 48, 4096, &timing[6], 1 },
};
...
static void nand_randomizer_init_by_chip(struct mtd_info *mtd, int chip)
{
	...
	unsigned int oob_size = 32; /* should be mtd->oobsize,
				     ...
				     */
	unsigned char id_data[NAND_ID_MAX_SIZE];
	int randomized;
	...
	nand_read_chip_id(mtd, id_data, info->read_id_bytes);
		...
		int data_size, int oob_size)
{
	...
	case STATE_PIO_WRITING:
		if (real_data_size > 0) {
			int i;
			void __iomem *wr_base = nand->mmio_base + NDDB;
			u32 *wr_buf = (u32 *)(nand->data_buff + nand->data_column);

			for (i=0; i<real_data_size; i++) {
				__raw_writel(wr_buf[i], wr_base);
			}
		}
		if (real_oob_size > 0) {
			int i;
			void __iomem *wr_base = nand->mmio_base + NDDB;
			u32 *wr_buf = (u32 *)(nand->oob_buff + nand->oob_column);

			for (i=0; i<real_oob_size; i++) {
				__raw_writel(wr_buf[i], wr_base);
			}
		}
		break;
	case STATE_PIO_READING:
		if (real_data_size > 0) {
			int i = 0;
			void __iomem *rd_base = nand->mmio_base + NDDB;
			u32 *rd_buf = (u32 *)(nand->data_buff + nand->data_column);
			for (i=0; i<real_data_size; i++) {
				rd_buf[i] = __raw_readl(rd_base);
			}
		}
		if (real_oob_size > 0) {
			int i = 0;
			void __iomem *rd_base = nand->mmio_base + NDDB;
			u32 *rd_buf = (u32 *)(nand->oob_buff + nand->oob_column);
			for (i=0; i<real_oob_size; i++) {
				rd_buf[i] = __raw_readl(rd_base);
			}
		}
		...
		uint16_t column, int page_addr)
{
	...
	case NAND_CMD_RNDOUT:
		...
		break;
	default:
		i = (int)((void *)(&nand->buf_start) - (void *)nand);
	...
	case NAND_CMD_READID:
		...
		nand->data_buff = (unsigned char *) nand->misc_pagebuf;
...
void mv88dexx_nand_dma_reset(struct pxa3xx_nand_info *info)
{
	SIE_BCMCFGW *cfgw;
	unsigned int i = 0;
	struct pxa3xx_nand *nand = info->nand_data;
	...
	case NAND_CMD_RNDOUT:
		if (info->n_planes == 2) {
			...
			oob_buff_phys	= nand->data_desc_addr + DMA_H_SIZE
				...
				+ (PXA3XX_NAND_MAX_OOBSIZE * 2) + info->page_size * 2;
			...
			oob_buff_phys	= nand->data_desc_addr + DMA_H_SIZE
				+ nand->chip_select * sizeof(struct nand_buffers)
				+ (PXA3XX_NAND_MAX_OOBSIZE * 2) + info->page_size;
	...
	if (use_dma) {
		...
		if (unlikely(buf == chip->buffers->databuf)) {
			nand->data_buff_phys = nand->data_desc_addr +
				DMA_H_SIZE +
				NAND_BUFFERS_TOTAL_SIZE * nand->chip_select +
				sizeof(struct nand_buffers) + PXA3XX_NAND_MAX_OOBSIZE * 2;
		...
		struct nand_chip *chip, const uint8_t *buf, int oob_required)
{
	...
	if (use_dma) {
		...
		nand->oob_buff_phys = nand->data_desc_addr + DMA_H_SIZE
				+ sizeof(struct nand_buffers) * nand->chip_select
				+ (PXA3XX_NAND_MAX_OOBSIZE * 2) + mtd->writesize;
	}
...
again:
	...
	if (nand->retcode == ERR_NONE)
		...
		default:
			...
			else if ((nand->bad_count < nand->nand_sts.err_l1) && (nand->bad_count >= 2)) {
				nand->nand_sts.page_count_err_l1++;
			} else if ((nand->bad_count < nand->nand_sts.err_l2) && (nand->bad_count >= nand->nand_sts.err_l1)){
...
static void free_dummy_rd_buf(struct pxa3xx_nand *nand)
{
	...
	if (nand->drd_buf != NULL) {
		unmap_addr(&nand->pdev->dev, nand->drd_dma_addr,
			nand->drd_buf, PXA3XX_NAND_MAX_PAGESIZE*2,
			DMA_FROM_DEVICE);
		kfree(nand->drd_buf);
		nand->drd_buf = NULL;
...
{
	if (nand->drd_buf == NULL) {
		nand->drd_buf = kmalloc(PXA3XX_NAND_MAX_PAGESIZE*2,
					GFP_KERNEL);
		if (nand->drd_buf == NULL) {
		...
		nand->drd_dma_addr = map_addr(nand,
			(void *)nand->drd_buf,
			PXA3XX_NAND_MAX_PAGESIZE*2, DMA_FROM_DEVICE);
...
int pxa3xx_dummy_read(struct mtd_info *mtd, int page)
{
	...
	mv88dexx_nand_dma_fill_read_desc(info, page,
			nand->drd_dma_addr,
			nand->drd_dma_addr + PXA3XX_NAND_MAX_PAGESIZE);
	mv88dexx_nand_dma_read_go(nand);
	ret = wait_for_completion_timeout(&nand->cmd_complete,
			CHIP_DELAY_TIMEOUT);
	if (!ret) {
		pxa3xx_nand_dump_registers(info);
		printk(KERN_ERR "pxa3xx_dummy_read  time out @ %x!!!\n", page);
	}
		...
		int page)
{
	...
	return pxa3xx_read_page(mtd, NULL, page);
}
...
static int pxa3xx_nand_scan_bbt(struct mtd_info *mtd)
{
	...
	if ((info->n_planes == PXA3XX_MTD_DUAL_PLANE) || info->eslc_mode)
		return 0;
...
static int pxa3xx_nand_get_mtd_info(struct mtd_info *mtd, loff_t off, loff_t *dst)
{
	...
	*dst = off;
	if (info->eslc_mode == 1) {
		...
		return PXA3XX_MTD_SLC;
	} else {
		return (info->n_planes == 1 ? PXA3XX_MTD_SINGLE_PLANE : PXA3XX_MTD_DUAL_PLANE);
	}
}
...
static void pb_init(struct pxa3xx_nand *nand)
{
	...
	pb_writel(nand, RA_pBridge_BCM + RA_BCM_base,
		((unsigned long)nand->mmio_phys & 0xF0000000));
				    ...
				    const struct pxa3xx_nand_flash *f)
{
	...
	info->read_id_bytes = f->id_act_len ? f->id_act_len : 4;
...
static int pxa3xx_nand_scan(struct mtd_info *mtd, int type_plane)
{
	...
	if (f->id_act_len == 6) {
		...
		id = (uint16_t *)(nand->data_buff);
		if (id[0] != 0)
			printk(KERN_INFO " 6 bytes id flash id %x, ext id %x, last id %x\n",
				id[0], id[1], id[2]);
		else {
			pxa3xx_nand_dump_registers(info);
			free_cs_resource(info, nand->chip_select);
	...
	pxa3xx_flash_ids[0].id[0] = (f->chip_id >> 0) & 0xff;
	pxa3xx_flash_ids[0].id[1] = (f->chip_id >> 8) & 0xff;
	pxa3xx_flash_ids[0].id[2] = (f->ext_id >> 0) & 0xff;
	pxa3xx_flash_ids[0].id[3] = (f->ext_id >> 8) & 0xff;
	pxa3xx_flash_ids[0].id_len = 4;
	pxa3xx_flash_ids[0].pagesize = f->page_size;
	pxa3xx_flash_ids[0].oobsize = 32;
	...
	chip->ecc.strength = f->ecc_strength;
	...
	if ((f->chip_id == 0xd7ec && f->ext_id == 0x7a94) ||
			...
			(f->chip_id == 0xf198 && f->ext_id == 0x1580) ||
	...
	if (retry->rr_type != NO_READ_RETRY) {
		if (berlin_nand_enhance_op_init(nand, pxa3xx_flash_ids) != 0) {
			   ...
			   char *buf)
{
	...
	size = sprintf(buf, "page RD cnt: \t[%d]\npage WR cnt: \t[%d]\n"
		"block erase cnt: \t[%d]\n",
		nand->nand_sts.page_readed, nand->nand_sts.page_writed,
		nand->nand_sts.block_erased);
	return size;
}
			   ...
			   char *buf)
{
	...
	size = sprintf(buf, "===== uncorrectable erro record =====\n"
	...
	while(pn) {
		...
		size = sprintf(buf, "%spage: [0x%x]\n",
			buf, nand->nand_sts.uncerr_pn_rec[i]);
		pn--;
	}
	return size;
}

static DEVICE_ATTR(ecc_sts, S_IRUGO | S_IWUSR | S_IWGRP, ecc_sts_show, ecc_sts_store);
static DEVICE_ATTR(rw_sts, S_IRUGO , rw_sts_show, NULL);
static DEVICE_ATTR(uncerr_rec, S_IRUGO , unc_err_rec_show, NULL);
...
static int alloc_nand_resource(struct platform_device *pdev)
{
	...
	nand->mmio_base = devm_ioremap_resource(&pdev->dev, r);
	if (!nand->mmio_base) {
		dev_err(&pdev->dev, "failed to map nfc reg memory\n");
		ret = -ENOMEM;
		goto fail_alloc;
	}
	nand->mmio_phys = (u32)r->start;
	...
	ret = request_irq(irq, mv88dexx_nand_dma_intr, 0,
			  pdev->name, nand);
	if (ret < 0) {
	...
	if (use_dma) {
		nand->data_desc = dma_alloc_coherent(&pdev->dev, DMA_H_SIZE
				+ NAND_BUFFERS_TOTAL_SIZE * pdata->cs_num,
	...
	nand->bounce_buffer = kmalloc(PXA3XX_NAND_MAX_PAGESIZE + PXA3XX_NAND_MAX_OOBSIZE, GFP_KERNEL);
	...
	nand->misc_pagebuf = kmalloc(PXA3XX_NAND_MAX_PAGESIZE + PXA3XX_NAND_MAX_OOBSIZE, GFP_KERNEL);
	if (nand->misc_pagebuf == NULL) {
		dev_err(&pdev->dev, "failed to allocate"
				" miscellaneous page buffer\n");
		ret = -ENOMEM;
		goto fail_free_bounce_buffer;
	}
	...
	for (cs = 0; cs < pdata->cs_num; cs++) {
		...
		if (use_dma)
			chip->buffers =
				...
				+ DMA_H_SIZE + NAND_BUFFERS_TOTAL_SIZE * cs);
		else
			chip->buffers = (struct nand_buffers *) kmalloc(NAND_BUFFERS_TOTAL_SIZE,
						GFP_KERNEL);

		chip->buffers->ecccalc = (uint8_t*)(chip->buffers + 1);
		chip->buffers->ecccode = chip->buffers->ecccalc + PXA3XX_NAND_MAX_OOBSIZE;
		chip->buffers->databuf = chip->buffers->ecccode + PXA3XX_NAND_MAX_OOBSIZE;
...
fail_free_buf:
	...
	kfree(nand->misc_pagebuf);

fail_free_bounce_buffer:
...
#endif /* CONFIG_BERLIN_NAND_RANDOMIZER */
#ifdef CONFIG_BERLIN_NAND_READ_RETRY
	berlin_nand_enhance_op_exit(nand);
#endif

	clk_disable_unprepare(pdata->clk_ecc);
	clk_disable_unprepare(pdata->clk_bus);
...
static struct pxa3xx_nand_platform_data* __init pxa3xx_nand_probe_dt(struct platform_device *pdev)
{
	...
	pdata->clk_ecc = devm_clk_get(dev, "ecc");
	if (!IS_ERR(pdata->clk_ecc))
		clk_prepare_enable(pdata->clk_ecc);

	pdata->clk_bus = devm_clk_get(dev, "bus");
	if (!IS_ERR(pdata->clk_bus))
		clk_prepare_enable(pdata->clk_bus);

	if (!of_get_property(np, "marvell,nfc-dma", NULL)) {
		use_dma = 0;
	}

	if (of_get_property(np, "marvell,nfc-naked-cmd", NULL))
		pdata->controller_attrs |= PXA3XX_NAKED_CMD_EN;

	if (of_get_property(np, "marvell,nfc-arbi", NULL))
		pdata->controller_attrs |= PXA3XX_ARBI_EN;

	if (of_get_property(np, "marvell,nfc-dual-chip", NULL))
		pdata->cs_num = 2;
	else
		pdata->cs_num = 1;
...
static int pxa3xx_nand_probe(struct platform_device *pdev)
{
	...
	for (cs = 0; cs < pdata->cs_num; cs++) {
...
#if (defined(CONFIG_MTD_CMDLINE_PARTS) || defined(CONFIG_MTD_OF_PARTS))
		{
			...
			nr_parts = parse_mtd_partitions(mtd, NULL, &parts, &ppdata);
			if (!nr_parts) {
			...
			if (plane_cmdline) {
				...
				printk(KERN_INFO "add [%d] sing plane parts\n", nr_parts);
