...
#include "pbridge_api_dhub.h"
#include "pBridge.h"

#define PXA3XX_NAND_MAX_OOBSIZE		744
#define PXA3XX_NAND_MAX_PAGESIZE	16384

#define PXA3XX_MTD_SINGLE_PLANE		1
#define PXA3XX_MTD_DUAL_PLANE		2
#define PXA3XX_MTD_ESLC			3
#define PXA3XX_MTD_SLC			4

#define	NAND_BUFFERS_TOTAL_SIZE		(sizeof(struct nand_buffers) + PXA3XX_NAND_MAX_OOBSIZE*3 + PXA3XX_NAND_MAX_PAGESIZE)
...
struct pxa3xx_nand_flash {
	...
	uint8_t		id_act_len;	/*Actually ID len reading when read ID,default 4*/
...
#define UNCERR_REC_DEPTH	64
struct pxa3xx_nand_sts {
	/* error bits number of level 1*/
	u16	err_l1;
	/* error bits number of level 2*/
	u16	err_l2;
	/* page count with error bit 0 */
	u32	page_count_err_0;
	/* page count with error bit 1 */
	u32	page_count_err_1;
	/* page count with error bits from 2 to l1*/
	u32	page_count_err_l1;
	/* page count with error bits from l1 to l2*/
	u32	page_count_err_l2;
	/* page count with error bits more than l2*/
	u32	page_count_err_l3;
	u32	uncorrect_page_cnt;
	u32     page_count_blank;
	/* total corrected bits number*/
	u32	corrected_err_bits;
	u32	block_erased;
	u32	page_readed;
	u32	page_writed;
	/* page number record for uncorrectable error*/
	u32	uncerr_pn_rec[UNCERR_REC_DEPTH];
	u16	rec_idx;
	u16	ecc_strength;
};
...
struct pxa3xx_nand {
	...
	struct pxa3xx_nand_sts	nand_sts;
...
extern void mv88dexx_nand_dma_reset(struct pxa3xx_nand_info *info);
