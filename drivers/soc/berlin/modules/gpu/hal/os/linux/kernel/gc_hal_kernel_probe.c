...
static int ionMemUsageMode = 2;
module_param(ionMemUsageMode, int, 0644);

static int sysMemUsageMode = 0;
module_param(sysMemUsageMode, int, 0644);
/*###end by marvell-bg2*/
    ...
    )
{
    ...
    /*###modified by marvell-bg2*/
    ionMemUsageMode    = Param->ionMemUsageMode;
    sysMemUsageMode    = Param->sysMemUsageMode;
    /*###end by marvell-bg2*/
}
...
/*###modified by marvell-bg2*/
    printk("  ionMemUsageMode    = %d\n",      ionMemUsageMode);
    printk("  sysMemUsageMode    = %d\n",      sysMemUsageMode);
/*###end by marvell-bg2*/
}
...
/* Modified for BG4CDPA0 */
#define CLEAN_CLOCK_BITS (~(7 << 7))
#define CLOCK_DOWN_BITS (5 << 7)
#define CLOCK_UP_BITS (1 << 7)


static gctPOINTER __gckGetClockAddress(void) {
    static gctPOINTER addr = NULL;
    if (addr == NULL) {
        addr = ioremap_nocache(0xf7ea062c, sizeof(long));
        printk("==Init ioremap==\n");
    }
    return addr;
}

static void __gckAdjustClock(int down) {
    gctPOINTER addr = __gckGetClockAddress();
    u32 val = readl(addr);
    val &= CLEAN_CLOCK_BITS;
    if (down) {
        val |= CLOCK_DOWN_BITS;
    }else{
        val |= CLOCK_UP_BITS;
...
/*###modified by marvell-bg2*/
    if (ioctlCode == IOCTL_GCHAL_SET_EXT_CLOCK ||
        ioctlCode == IOCTL_GCHAL_GET_EXT_CLOCK)
    {
        gcsHAL_ExtClock extClk;
        copyLen = copy_from_user(&extClk, (void *)arg, sizeof(gcsHAL_ExtClock));
        if (copyLen != 0)
        {
            gcmkTRACE_ZONE(
                gcvLEVEL_ERROR, gcvZONE_DRIVER,
        ...
        if ((extClk.eFlags & gcvEXT_PM_MASK) == 0 ||
            (extClk.eFlags & ~gcvEXT_PM_MASK) != 0)
        {
            printk(KERN_ERR "gfx driver: invalid power managment flags 0x%x\n", extClk.eFlags);
            gcmkONERROR(gcvSTATUS_INVALID_ARGUMENT);
        }

        if (ioctlCode == IOCTL_GCHAL_SET_EXT_CLOCK)
        {
            gcmkONERROR(platform.ops->setClockExt(&platform, &extClk));
        }
        else
        {
            gcmkONERROR(platform.ops->getClockExt(&platform, &extClk));
            copyLen = copy_to_user((void *)arg, &extClk, sizeof(gcsHAL_ExtClock));
            if (copyLen != 0)
            {
                gcmkTRACE_ZONE(
                    gcvLEVEL_ERROR, gcvZONE_DRIVER,
...
/*###end by marvell-bg2*/
    if ((ioctlCode == IOCTL_GCHAL_CLOCK_UP)
            || (ioctlCode == IOCTL_GCHAL_CLOCK_DOWN))
    {
        gckOS_AcquireMutex(device->kernels[gcvCORE_MAJOR]->hardware->os,
                device->kernels[gcvCORE_MAJOR]->hardware->powerMutex, gcvINFINITE);
        printk("==GFX 3d core clk %s\n", ioctlCode == IOCTL_GCHAL_CLOCK_UP ? "up" : "down");
        if (ioctlCode == IOCTL_GCHAL_CLOCK_UP) {
            __gckAdjustClock(0);
        }else{
            __gckAdjustClock(1);
        }
        gckOS_ReleaseMutex(device->kernels[gcvCORE_MAJOR]->hardware->os,
                device->kernels[gcvCORE_MAJOR]->hardware->powerMutex);
        return 0;
    }
...
/*###modified by marvell-bg2*/
    /* create sysfs device after everything is ready */
...
#endif
{
    ...
    gcsMODULE_PARAMETERS moduleParam = {
        ...
        /*###modified by marvell-bg2*/
        .ionMemUsageMode     = ionMemUsageMode,
        .sysMemUsageMode     = sysMemUsageMode,
        /*###end by marvell-bg2*/
    };
...
MODULE_DEVICE_TABLE(of, gpu_match);
