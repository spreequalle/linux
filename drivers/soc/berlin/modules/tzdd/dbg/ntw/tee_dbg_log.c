...
#define TEE_DBG_CMD_TBL_LEN (100)
typedef struct _tee_dbg_idx_table_item_t {
	uint32_t cmd_idx;
	uint32_t s;
	uint32_t ns;
} tee_dbg_idx_table_item_t;
typedef struct _tee_dbg_idx_table_t {
	uint32_t start_idx;
	osa_mutex_t lock;
	tee_dbg_idx_table_item_t data[TEE_DBG_CMD_TBL_LEN];
} tee_dbg_idx_table_t;
...
static tee_dbg_idx_table_t g_tee_cmd_id_tbl;

void tee_dbg_log_init(ulong_t buffer, ulong_t ctl)
{
	memset(&g_tee_cmd_id_tbl, sizeof(g_tee_cmd_id_tbl), 0);
	g_tee_cmd_id_tbl.lock = osa_create_mutex();
...
void tee_dbg_log_cmd(void)
{
	static uint32_t idx = 0;
	struct timespec time;

	osa_wait_for_mutex(g_tee_cmd_id_tbl.lock, INFINITE);
	g_tee_cmd_id_tbl.data[g_tee_cmd_id_tbl.start_idx].cmd_idx = idx++;
	getrawmonotonic(&time);
	g_tee_cmd_id_tbl.data[g_tee_cmd_id_tbl.start_idx].s = time.tv_sec;
	g_tee_cmd_id_tbl.data[g_tee_cmd_id_tbl.start_idx].ns = time.tv_nsec;
	g_tee_cmd_id_tbl.start_idx++;
	g_tee_cmd_id_tbl.start_idx %= TEE_DBG_CMD_TBL_LEN;
	osa_release_mutex(g_tee_cmd_id_tbl.lock);
}
static int cmd_tbl_seq_show(struct seq_file *m, void *v)
{
	uint32_t i = 0;
	uint32_t cur_idx;
	static bool log_wraped = false;

	osa_wait_for_mutex(g_tee_cmd_id_tbl.lock, INFINITE);
	if (log_wraped) {
		cur_idx = g_tee_cmd_id_tbl.start_idx;
	} else {
		cur_idx = 0;
	}
	for (i = 0; i < TEE_DBG_CMD_TBL_LEN; i++) {
		if (g_tee_cmd_id_tbl.data[cur_idx].cmd_idx == 0 &&
			g_tee_cmd_id_tbl.data[cur_idx].s == 0 &&
			g_tee_cmd_id_tbl.data[cur_idx].ns == 0) {
			break;
		}
		seq_printf(m, "%08x:%08x:%08x\n", g_tee_cmd_id_tbl.data[cur_idx].cmd_idx,
		   g_tee_cmd_id_tbl.data[cur_idx].s, g_tee_cmd_id_tbl.data[cur_idx].ns);
		if (++cur_idx >= TEE_DBG_CMD_TBL_LEN) {
			cur_idx %= TEE_DBG_CMD_TBL_LEN;
			log_wraped = true;
		}
	}
	osa_release_mutex(g_tee_cmd_id_tbl.lock);
...
static int cmd_tbl_open(struct inode *inode, struct file *file)
{
	return single_open(file, cmd_tbl_seq_show, NULL);
}

static const struct file_operations cmd_tbl_file_ops = {
	.owner = THIS_MODULE,
	.open = cmd_tbl_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
void tee_dbg_proc_fs_init(void)
{
	...
	proc_create("idx", S_IRUGO, _g_tzdd_entry, &cmd_tbl_file_ops);
}
...
void tee_dbg_proc_fs_cleanup(void)
{
	if (_g_tzdd_entry) {
		...
		remove_proc_entry("idx", _g_tzdd_entry);
