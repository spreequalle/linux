...
typedef struct _tzdd_ctx_node_t
{
	uint8_t magic[4];
...
typedef struct _tzdd_shm_node_t
{
	uint8_t magic[4];
	tee_impl tee_ctx_ntw;
...
typedef struct _tzdd_ss_node_t
{
	uint8_t magic[4];
	tee_impl tee_ctx_ntw;
	tee_impl tee_ss_ntw;
...
extern long tzdd_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
