...
#ifndef __LINUX_KERNEL__
MV_OSAL_HANDLE_MUTEX_t gBCM_SCHED_lock;
#endif

void BCM_SCHED_Open(void)
{
#ifndef __LINUX_KERNEL__
	if (gBCM_SCHED_lock == NULL) {
		MV_OSAL_Mutex_Create(&gBCM_SCHED_lock);
	}
#endif
}
...
#ifndef __LINUX_KERNEL__
	if (gBCM_SCHED_lock) {
		MV_OSAL_Mutex_Destroy(&gBCM_SCHED_lock);
		gBCM_SCHED_lock = NULL;
	}
#endif
}
...
int BCM_SCHED_AutoPushCmd(UNSG32 QID, UNSG8 uchEnable)
{
	...
	GA_REG_WORD32_READ(MEMMAP_AVIO_BCM_REG_BASE + RA_AVIO_BCM_AUTOPUSH,
			   &uiBcmQ_AutoPushSts);
...
int BCM_SCHED_PushCmd(UNSG32 QID, UNSG32 * pCmd, UNSG32 * cfgQ)
{
	...
	if (pCmd) {
		...
		if (cfgQ) {
			...
			MV_OSAL_Mutex_Lock(gBCM_SCHED_lock);
#endif
			GA_REG_WORD32_WRITE(MEMMAP_AVIO_BCM_REG_BASE + addr,
					    pCmd[0]);
			GA_REG_WORD32_WRITE(MEMMAP_AVIO_BCM_REG_BASE + addr + 4,
					    pCmd[1]);
#ifndef __LINUX_KERNEL__
			MV_OSAL_Mutex_Unlock(gBCM_SCHED_lock);
#endif
...
void BCM_SCHED_GetEmptySts(UNSG32 QID, UNSG32 * EmptySts)
{
	...
	GA_REG_WORD32_READ(MEMMAP_AVIO_BCM_REG_BASE + RA_AVIO_BCM_EMP_STS,
			   &bcmQ_sts);
	if (bcmQ_sts & (1 << QID)) {
		*EmptySts = 1;
	} else {
