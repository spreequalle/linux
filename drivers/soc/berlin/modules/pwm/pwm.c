...
static void __iomem *pwmch_base;
				...
				unsigned int cmd, unsigned long arg)
{
	...
	case PWM_IOCTL_READ:
		...
		pwm_rw.data = readl_relaxed(pwmch_base + (pwmch*0x10) + pwm_rw.offset);
	...
	case PWM_IOCTL_WRITE:
		...
		writel_relaxed(pwm_rw.data, pwmch_base + (pwmch*0x10) + pwm_rw.offset);
		break;
	default:
...
static int pwmch_suspend(struct device *dev)
{
	...
	for (i = 0; i < GALOIS_PWMCH_NUM; i++) {
		saved_pwm_status[i].control = readl_relaxed(pwmch_base + (i * 0x10) + CONTROL_REG_OFFSET);
		saved_pwm_status[i].duty = readl_relaxed(pwmch_base + (i * 0x10) + DUTY_REG_OFFSET);
		saved_pwm_status[i].termcnt = readl_relaxed(pwmch_base + (i * 0x10) + TERMCNT_REG_OFFSET);
		saved_pwm_status[i].enable = readl_relaxed(pwmch_base + (i * 0x10) + ENABLE_REG_OFFSET);
	}
...
static int pwmch_resume(struct device *dev)
{
	...
	for (i = 0; i < GALOIS_PWMCH_NUM; i++) {
		writel_relaxed(saved_pwm_status[i].control, pwmch_base + (i * 0x10) + CONTROL_REG_OFFSET);
		writel_relaxed(saved_pwm_status[i].duty, pwmch_base + (i * 0x10) + DUTY_REG_OFFSET);
		writel_relaxed(saved_pwm_status[i].termcnt, pwmch_base + (i * 0x10) + TERMCNT_REG_OFFSET);
		writel_relaxed(saved_pwm_status[i].enable, pwmch_base + (i * 0x10) + ENABLE_REG_OFFSET);
	}
...
static int pwmch_probe(struct platform_device *pdev)
{
	int i;
	struct resource *r;

	r = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	pwmch_base = devm_ioremap_resource(&pdev->dev, r);
	if (IS_ERR(pwmch_base))
		return PTR_ERR(pwmch_base);
...
static int pwmch_remove(struct platform_device *pdev)
{
	int i;
