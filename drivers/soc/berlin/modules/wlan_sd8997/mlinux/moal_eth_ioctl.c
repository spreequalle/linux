 ...
 * @param data_len       Length of the arguments buffer
 * @param total_num_args Pointer to the number of arguments extracted
 *
 * @return               MLAN_STATUS_SUCCESS on success, MLAN_STATUS_FAILURE
 *                       with invalid input.
 */
static mlan_status
parse_arguments(t_u8 *pos, int *data, size_t data_len, int *total_num_args)
{
	size_t pos_idx;  // Index into pos
	size_t num_parsed_args = 0;  // Number of arguments parsed
	size_t arg_idx = 0;  // Index into current argument
	char cur_arg[10] = { 0 };  // Current argument
	...
	for (pos_idx = 0; pos_idx <= pos_len; ++pos_idx) {
		// The end of the current argument
		if (pos[pos_idx] == '\0' || pos[pos_idx] == ' ') {
			// Ignore if there isn't any space left
			if (num_parsed_args >= data_len) {
				++num_parsed_args;
				break;
			}
			BUG_ON(arg_idx >= sizeof(cur_arg) + 1);
			if (woal_atoi(&data[num_parsed_args], cur_arg) ==
					MLAN_STATUS_FAILURE) {
			...
			// Reset state for the next argument
			arg_idx = 0;
			memset(cur_arg, 0, sizeof(cur_arg));
		} else {
			// Argument too big
			if (arg_idx >= sizeof(cur_arg)) {
				PRINTM(MERROR, "Argument too large\n");
				return MLAN_STATUS_FAILURE;
			}

			cur_arg[arg_idx] = pos[pos_idx];
			++arg_idx;
		}
	}

	*total_num_args = num_parsed_args;
	return MLAN_STATUS_SUCCESS;
}
...
woal_priv_signalext_cfg(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	...
	if (strlen(respbuf) == header_len) {
		...
		/* SET operation */
		if (parse_arguments(respbuf + header_len, &enable, 1,
					&user_data_len) ==
				MLAN_STATUS_FAILURE) {
			ret = -EINVAL;
			goto done;
...
woal_priv_atim_window(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	...
	if (strlen(respbuf) == header_len) {
		...
		/* SET operation */
		if (parse_arguments(respbuf + header_len, &atim, 1,
					&user_data_len) ==
				MLAN_STATUS_FAILURE) {
			ret = -EINVAL;
			goto done;
...
woal_priv_tdls_cs_chan(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	...
	if (strlen(respbuf) == header_len) {
		...
		/* SET operation */
		if (parse_arguments(respbuf + header_len + 1, &data, 1,
					&user_data_len) ==
				MLAN_STATUS_FAILURE) {
			ret = -EINVAL;
			goto done;
...
woal_priv_p2p_ecsa(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	...
	if (parse_arguments(respbuf + header_len + 1, data, ARRAY_SIZE(data),
				&user_data_len) == MLAN_STATUS_FAILURE) {
		ret = -EINVAL;
		goto done;
	}
	...
	woal_priv_get_nonglobal_operclass_by_bw_channel(priv, bw, channel,
							&oper_class);
	if (oper_class == 0) {
		PRINTM(MERROR, "Wrong oper class!\n");
		ret = -EFAULT;
		goto done;
	}
	ext_chan_switch->element_id = EXTEND_CHANNEL_SWITCH_ANN;
