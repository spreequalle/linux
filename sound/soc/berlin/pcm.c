                                  ...
                                  struct device_attribute *attr, char *buf)
{
	...
	data_or_error = i2c_smbus_read_byte_data(tas5720_client, TAS5720_SADDR_FAULT);
	if (data_or_error < 0)
                                     ...
                                     struct device_attribute *attr, char *buf)
{
	...
	if (chip == NULL) {
        ...
        data_or_error = i2c_smbus_read_byte_data(tas5720_client, TAS5720_SADDR_POWER);
	if (data_or_error & TAS5720_POWER_SDZ)
                                      ...
                                      size_t count)
{
	...
	if (strncmp(buf, kTas5720SysfsShutdown,
		...
		data = TAS5720_POWER_SHUTDOWN;
		snd_printk("Shutting down TAS5720\n");
	} else {
		data = TAS5720_POWER_RESET;
	...
	ret = i2c_smbus_write_byte_data(tas5720_client, TAS5720_SADDR_FAULT,
	                                0x00);
	if (ret < 0)
		snd_printk("Error writing FAULT to 0x00: %d", ret);

	ret = i2c_smbus_write_byte_data(tas5720_client, TAS5720_SADDR_POWER,
	                                data);
	if (ret < 0) {
		snd_printk("Error writing POWER to 0x%X: %d", data, ret);
		return ret;
	}
