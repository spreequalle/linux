...
woal_start_ft_roaming(moal_private *priv, mlan_ssid_bssid *ssid_bssid)
{
	...
	chan.band = (ssid_bssid->channel < 36) ?
		IEEE80211_BAND_2GHZ : IEEE80211_BAND_5GHZ;
	chan.center_freq =
		ieee80211_channel_to_frequency(ssid_bssid->channel, chan.band);
			 ...
			 t_u16 reason_code)
{
	...
	ENTER();
	PRINTM(MMSG, "Received disassociation request on %s, reason code %d\n",
			       ...
			       t_u16 status_code, struct sk_buff *skb)
{
	...
	if (action_code == WLAN_TDLS_SETUP_REQUEST ||
	    action_code == WLAN_TDLS_SETUP_RESPONSE) {
		/* wmm info */
		woal_add_wmm_ie(priv, skb, WMM_TYPE_INFO);
...
woal_register_cfg80211(moal_private *priv)
{
	...
	wiphy->regulatory_flags |=
		REGULATORY_DISABLE_BEACON_HINTS | REGULATORY_COUNTRY_IE_IGNORE;
#endif
