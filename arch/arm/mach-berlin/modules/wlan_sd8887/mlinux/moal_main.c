...
#if defined(STA_WEXT) && defined(UAP_WEXT) && defined(STA_CFG80211) && \
	    defined(UAP_CFG80211)
int cfg80211_wext = STA_WEXT_MASK | UAP_WEXT_MASK | STA_CFG80211_MASK| UAP_CFG80211_MASK;
#elif defined(STA_WEXT) || defined(UAP_WEXT)
...
/** Semaphore to ensure WiFi is initialized prior to Bluetooth */
unsigned long mlan_WiFiInitSem;
struct semaphore gWiFiInitSem;
/** Semaphore for user init data, eg: cal data */
struct semaphore gUserInitDataSem;
...
woal_set_user_init_data(moal_handle *handle, int type, t_u8 wait_option)
{
	...
	if (type == INIT_CFG_DATA) {
		PRINTM(MMSG,"Loading init config data: %s\n",init_cfg);
		if (req_fw_nowait) {
			MOAL_ACQ_SEMAPHORE_BLOCK(&gUserInitDataSem);
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 32)
...
#endif
#endif
				MOAL_REL_SEMAPHORE(&gUserInitDataSem);
				PRINTM(MERROR,
				       "Init config file request_firmware_nowait(%s) failed\n",init_cfg);
				goto done;
			}
		...
		if ((request_firmware
			...
			PRINTM(MERROR,
			       "Init config file request_firmware(%s) failed\n",init_cfg);
			goto done;
		}
		}
	} else if (type == TXPWRLIMIT_CFG_DATA) {
		PRINTM(MMSG,"Loading tx power limit config: %s\n",txpwrlimit_cfg);
		if (req_fw_nowait) {
			MOAL_ACQ_SEMAPHORE_BLOCK(&gUserInitDataSem);
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 32)
			if ((request_firmware_nowait
			...
			if ((request_firmware_nowait
				...
				MOAL_REL_SEMAPHORE(&gUserInitDataSem);
				PRINTM(MERROR,
				       "txpwrlimit config file request_firmware_nowait(%s) failed\n",txpwrlimit_cfg);
				goto done;
			}
			handle->init_user_conf_wait_flag = MFALSE;
			wait_event_interruptible(handle->init_user_conf_wait_q,
		...
		if ((request_firmware
		     (&handle->user_data, txpwrlimit_cfg,
		      handle->hotplug_device)) < 0) {
			PRINTM(MERROR,
				       "txpwrlimit config file request_firmware(%s) failed\n",txpwrlimit_cfg);
			goto done;
		}
		}
	} else if (type == COUNTRY_POWER_TABLE) {
		PRINTM(MMSG,"Loading country power table: %s\n",country_txpwrlimit);
		if (req_fw_nowait) {
			MOAL_ACQ_SEMAPHORE_BLOCK(&gUserInitDataSem);
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 32)
			if ((request_firmware_nowait
			...
			if ((request_firmware_nowait
				...
				MOAL_REL_SEMAPHORE(&gUserInitDataSem);
				PRINTM(MERROR,
				       "country txpwrlimit config file request_firmware_nowait(%s) failed\n",country_txpwrlimit);
				goto done;
			}
			handle->init_user_conf_wait_flag = MFALSE;
			wait_event_interruptible(handle->init_user_conf_wait_q,
				...
				if (status == -ENOENT) {
					PRINTM(MIOCTL,
							"Country power table file %s does not exist\n",country_txpwrlimit);
					ret = MLAN_STATUS_SUCCESS;
				} else if (status) {
					PRINTM(MERROR,
							"country txpwrlimit config file request_firmware(%s) failed\n",country_txpwrlimit);
					goto done;
				}
			}
	} else if (type == INIT_HOSTCMD_CFG_DATA) {
		PRINTM(MMSG,"Loading init hostcmd config data: %s\n",init_hostcmd_cfg);
		if (req_fw_nowait) {
			MOAL_ACQ_SEMAPHORE_BLOCK(&gUserInitDataSem);
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 32)
			if ((request_firmware_nowait
			...
			if ((request_firmware_nowait
				...
				MOAL_REL_SEMAPHORE(&gUserInitDataSem);
				PRINTM(MERROR,
				       "Init hostcmd config file request_firmware_nowait(%s) failed\n",init_hostcmd_cfg);
				goto done;
			}
			handle->init_user_conf_wait_flag = MFALSE;
			wait_event_interruptible(handle->init_user_conf_wait_q,
		...
		if ((request_firmware
		     (&handle->user_data, init_hostcmd_cfg,
		      handle->hotplug_device)) < 0) {
			PRINTM(MERROR,
				       "Init hostcmd config file request_firmware(%s) failed\n",init_hostcmd_cfg);
			goto done;
		}
	}
	}
...
woal_add_card_dpc(moal_handle *handle)
{
	...
	for (i = 0; i < handle->drv_mode.intf_num; i++) {
		...
		if (!woal_add_interface
		    (handle, handle->priv_num,
		     handle->drv_mode.bss_attr[i].bss_type)) {
			PRINTM(MFATAL, "%s: woal_add_interface failure\n", __func__);
			ret = MLAN_STATUS_FAILURE;
			goto err;
		}
	}
	...
	/* Avoiding setting of antcfg as mfg firmware
	*  currently does not support this option */
	if (mfg_mode == MLAN_INIT_PARA_ENABLED)
		PRINTM(MMSG,"mfg_mode set, skipping antcfg setting!\n");
	else if (antcfg && (handle->card_type == CARD_TYPE_SD8887)){
		if (MLAN_STATUS_SUCCESS !=
...
woal_init_fw_dpc(moal_handle *handle)
{
	...
	if (req_fw_nowait) {
		MOAL_REL_SEMAPHORE(&gUserInitDataSem);
		//CalData is downloaded. Signalling BT to resume init.
		MOAL_REL_SEMAPHORE(&gWiFiInitSem);
	}
	LEAVE();
	return ret;
}
...
woal_request_fw_dpc(moal_handle *handle, const struct firmware *firmware)
{
	...
	if (ret) {
		PRINTM(MFATAL, "%s: woal_init_fw_dpc failed: %d\n",
                        __func__, ret);
		goto done;
	}
	...
	if (ret) {
		PRINTM(MFATAL, "%s: woal_add_card_dpc failed: %d\n",
                        __func__, ret);
		goto done;
	}
done:
...
woal_request_fw(moal_handle *handle)
{
	...
	PRINTM(MINFO,"%s(): Attempting to load %s\n",__FUNCTION__,
			handle->drv_mode.fw_name);

	if (req_fw_nowait) {
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 32)
		PRINTM(MERROR, " %s():%d Downloading WiFi FW\n",__FUNCTION__,__LINE__);
		...
		PRINTM(MERROR, " %s():%d Downloading WiFi FW(sync)\n",__FUNCTION__,__LINE__);
...
#define MAX_WMM_QUEUE   4
	...
	if (bss_type == MLAN_BSS_TYPE_UAP) {
		if (MLAN_STATUS_SUCCESS != woal_init_uap_dev(dev, priv)) {
			PRINTM(MERROR, "%s: woal_init_uap_dev failed\n",
			       __func__);
			goto error;
		}
	}
	...
	PRINTM(MMSG, "%s: %s: Marvell 802.11 Adapter\n", __func__, dev->name);
	...
	if (bss_type == MLAN_BSS_TYPE_STA ||
		...
		for (i = 0; i < handle->histogram_table_num; i++)
			priv->hist_data[i] =
				kmalloc(sizeof(hgm_data), GFP_KERNEL);
		if (priv->hist_data)
 ...
 */
void
woal_request_fw_reload(moal_handle *handle)
{
	int intf_num;
	int ret = 0;
	mlan_ioctl_req *req = NULL;
	...
	ENTER();
	if (fw_serial) {
		PRINTM(MERROR, "We don't support fw reload in serail mode\n");
		LEAVE();
		return;
	}
	if ((handle->card_type != CARD_TYPE_SD8887) &&
...
woal_init_module(void)
{
	...
	MOAL_INIT_SEMAPHORE(&gWiFiInitSem);
	mlan_WiFiInitSem = (unsigned long) &gWiFiInitSem;
	MOAL_ACQ_SEMAPHORE_BLOCK(&gWiFiInitSem);

	MOAL_INIT_SEMAPHORE(&gUserInitDataSem);
...
MODULE_PARM_DESC(antcfg,
		 "0:default; 1:Tx/Rx antenna 1, 2:Tx/Rx antenna 2, 0xffff:enable antenna diversity");
