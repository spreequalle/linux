...
memory_type_mapping mem_type_mapping_tbl_bt[] = {
	{"ITCM", NULL, NULL, 0xF0},
	{"DTCM", NULL, NULL, 0xF1},
 ...
 */
rdwr_status
woal_cmd52_rdwr_firmware_bt(bt_private *priv, u8 doneflag)
{
	int ret = 0;
	int tries = 0;
	u8 ctrl_data = 0;
...
bt_dump_firmware_info_v2(bt_private *priv)
{
	...
	if (RDWR_STATUS_FAILURE == woal_cmd52_rdwr_firmware_bt(priv, doneflag))
		goto done;
	...
	for (idx = 0; idx < dump_num; idx++) {
		if (RDWR_STATUS_FAILURE ==
		    woal_cmd52_rdwr_firmware_bt(priv, doneflag))
			goto done;
		memory_size = 0;
		reg = dbg_dump_start_reg;
		for (i = 0; i < 4; i++) {
		...
		if (memory_size == 0) {
			...
			PRINTM(MSG, "%s_SIZE=0x%x\n",
			       mem_type_mapping_tbl_bt[idx].mem_name, memory_size);
			mem_type_mapping_tbl_bt[idx].mem_Ptr =
				vmalloc(memory_size + 1);
			if ((ret != BT_STATUS_SUCCESS) ||
			    !mem_type_mapping_tbl_bt[idx].mem_Ptr) {
				PRINTM(ERROR,
				       "Error: vmalloc %s buffer failed!!!\n",
				       mem_type_mapping_tbl_bt[idx].mem_name);
				goto done;
			}
			dbg_ptr = mem_type_mapping_tbl_bt[idx].mem_Ptr;
			end_ptr = dbg_ptr + memory_size;
		}
		doneflag = mem_type_mapping_tbl_bt[idx].done_flag;
		PRINTM(MSG, "Start %s output, please wait...\n",
		       mem_type_mapping_tbl_bt[idx].mem_name);
		do {
			stat = woal_cmd52_rdwr_firmware_bt(priv, doneflag);
			...
			if (RDWR_STATUS_DONE == stat) {
				...
				if (BT_STATUS_SUCCESS !=
				    bt_save_dump_info_to_file((char *)path_name,
							      (char *)file_name,
							      mem_type_mapping_tbl_bt
							      [idx].mem_Ptr,
							      memory_size))
					PRINTM(MSG,
					       "Can't save dump file %s in %s\n",
					       file_name, path_name);
				vfree(mem_type_mapping_tbl_bt[idx].mem_Ptr);
				mem_type_mapping_tbl_bt[idx].mem_Ptr = NULL;
				break;
			}
		} while (1);
	}
	...
	for (idx = 0; idx < dump_num; idx++) {
		if (mem_type_mapping_tbl_bt[idx].mem_Ptr) {
			vfree(mem_type_mapping_tbl_bt[idx].mem_Ptr);
			mem_type_mapping_tbl_bt[idx].mem_Ptr = NULL;
		}
	}
...
bt_set_evt_filter(bt_private *priv)
{
	...
	priv->bt_dev.sendcmdflag = TRUE;
	priv->bt_dev.send_cmd_ocf = BT_CMD_SET_EVT_FILTER;
	priv->adapter->cmd_complete = FALSE;
	wake_up_interruptible(&priv->MainThread.waitQ);
	if (!os_wait_interruptible_timeout(priv->adapter->cmd_wait_q,
					   priv->adapter->cmd_complete,
...
bt_enable_write_scan(bt_private *priv)
{
	...
	priv->bt_dev.sendcmdflag = TRUE;
	priv->bt_dev.send_cmd_ocf = BT_CMD_ENABLE_WRITE_SCAN;
	priv->adapter->cmd_complete = FALSE;
	wake_up_interruptible(&priv->MainThread.waitQ);
	if (!os_wait_interruptible_timeout(priv->adapter->cmd_wait_q,
					   priv->adapter->cmd_complete,
...
bt_enable_device_under_testmode(bt_private *priv)
{
	...
	PRINTM(CMD, "Queue enable device under testmode Command(0x%x)\n",
	       pcmd->ocf_ogf);
	priv->bt_dev.sendcmdflag = TRUE;
	priv->bt_dev.send_cmd_ocf = BT_CMD_ENABLE_DEVICE_TESTMODE;
	priv->adapter->cmd_complete = FALSE;
	wake_up_interruptible(&priv->MainThread.waitQ);
	if (!os_wait_interruptible_timeout(priv->adapter->cmd_wait_q,
					   priv->adapter->cmd_complete,
