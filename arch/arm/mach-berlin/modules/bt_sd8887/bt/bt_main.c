 ...
 */
static int
bt_read_firmware_status(bt_private *priv, u16 * dat)
{
	int ret = BT_STATUS_SUCCESS;
	...
	ret = sd_read_reg(priv, card_fw_status0_reg, &fws0);
	if (ret < 0) {
		LEAVE();
		return BT_STATUS_FAILURE;
	}
	ret = sd_read_reg(priv, card_fw_status1_reg, &fws1);
	if (ret < 0) {
		LEAVE();
		return BT_STATUS_FAILURE;
	}
...
bt_reload_fw(bt_private *priv)
{
    ...
    /** check fireware status to make sure chip is wake up */
	for (tries = 0; tries < FW_POLL_TRIES; ++tries) {
		ret = bt_read_firmware_status(priv, &firmwarestat);
