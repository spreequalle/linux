...
static int yaffs_readpage_nolock(struct file *f, struct page *pg)
{
	...
	obj = yaffs_inode_to_obj(file_inode(f));
...
#endif
{
	struct yaffs_obj *obj = yaffs_inode_to_obj(file_inode(file));
...
#endif
{
	...
	obj = yaffs_inode_to_obj(file_inode(file));
#else
...
static void yaffs_flush_inodes(struct super_block *sb)
{
	...
	list_for_each_entry(iptr, &sb->s_inodes, i_sb_list) {
		...
		if (iptr->i_state & (I_FREEING|I_CLEAR|I_WILL_FREE|I_NEW)) {
